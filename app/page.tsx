...
export default function HomePage() {
  ...
  return (
    <div className="min-h-dvh min-h-screen flex flex-col md:flex-row bg-gradient-to-br from-[#e8e4df] via-[#e5ddd5] to-[#ddd8d0] overflow-x-hidden pt-[env(safe-area-inset-top)] pl-[env(safe-area-inset-left)] pr-[env(safe-area-inset-right)] pb-[env(safe-area-inset-bottom)]">
      <main className="flex-1 flex flex-col items-center justify-center p-4 sm:p-8 min-w-0 min-h-0 overflow-y-auto overflow-x-hidden order-1 md:order-2">
      ...
      <aside className="flex-shrink-0 flex flex-col bg-white/80 backdrop-blur-sm border-t md:border-t-0 md:border-r border-white/50 shadow-sm min-h-0 order-2 md:order-1 w-full md:w-72 md:max-h-none max-h-64">
        <div className="p-4 border-b border-black/5 shrink-0">
          <div className="flex items-center gap-2">
            <MessageCircle className="size-5 text-[#00B900]" />
            <h2 className="font-semibold text-gray-800">トーク一覧・サンプル</h2>
          </div>
        </div>
